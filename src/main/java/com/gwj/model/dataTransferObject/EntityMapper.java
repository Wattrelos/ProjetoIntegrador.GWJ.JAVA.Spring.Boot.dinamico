...
public class EntityMapper {
    ...
    /**
     * Preenche a entidade com base nos parâmetros do request com conversão de
     * tipos.
     */
    public static <T> T fillEntity(T entity, HttpServletRequest request) {
        ...
        for (Method method : clazz.getMethods()) {
            ...
            if (method.getName().startsWith("set") && method.getParameterCount() == 1) {
                ...
                // Isso significa que, se o usuário apagar um texto no formulário (enviando ""),
                // o setter não será chamado. Como o setter não é chamado, o atributo da
                // entidade permanece nulo
                // Se você quiser permitir que o usuário limpe um campo de texto, mude a
                // verificação para permitir Strings vazias, mas bloqueie apenas o que realmente
                // não veio no request:
     ...
     * 
     * A partir do Java 17/21, você pode usar o switch diretamente com objetos
     * Class.
    ...
    private static Object convertValue(String value, Class<?> targetType, String fieldName) {
        ...
        return switch (targetType) {
            ...
            case Class<?> t when t == boolean.class || t == Boolean.class ->
                value.equalsIgnoreCase("true") || value.equalsIgnoreCase("on") || value.equals("1");
            case Class<?> t when t == BigDecimal.class -> {
                String cleanValue = value.replaceAll("[R$\\s]", "");
                // Tratamento PT-BR: Remove pontos (milhar) e troca a vírgula por ponto
                // (decimal)
